...
if($_POST['Submit']=='Save'){
	...
	if ($_FILES["visiting_card"]["size"]>2097152){
		...
		$res_q = mysqli_query($link1,"update sf_lead_master set partyid='".$party_id."', party_address='".cleanData($party_add)."', intial_remark='".$remark."', priority='".$priority."', vcard_url='".$path1."', sales_executive='".$designation."', dept_id='".$dept."', party_state='".$locationstate."', party_city='".$locationcity."', party_country='".$circle."', lead_source='".$source."', update_by='".$_SESSION['userid']."' where lid='".$lrow['lid']."'");
		if($res_q){	
			...
			//$msgg="Lead is edited successfully with reference <font size='4'>".$lrow['reference']."</font>";
            ...
            <div class="col-md-6"><label class="col-md-6 control-label">Transfer To <span class="red_small">*</span></label>
              <div class="col-md-6">
                 <select name="dept" class="form-control" id="dept" required>
            		...
            		<option value="<?php echo $drow['empid'];?>"<?php if($drow['empid']==$lrow['dept_id']){echo "selected='selected'";}?>><?php echo ucwords($drow['empname']); ?></option>
            		<?php } ?>
             	</select> 
              </div>
            </div>
