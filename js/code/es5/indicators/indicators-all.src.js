...
var SMAIndicator_spreadArray = (undefined && undefined.__spreadArray) || function (to, from, pack) {
    if (pack || arguments.length === 2) for (var i = 0, l = from.length, ar; i < l; i++) {
        if (ar || !(i in from)) {
            if (!ar) ar = Array.prototype.slice.call(from, 0, i);
            ar[i] = from[i];
        ...
        if (oldDataLength &&
            ...
            if (indicator.cropped) {
                ...
                var croppedData = indicator.cropData(table,
                    min,
                    max);
                var keys = SMAIndicator_spreadArray(['x'], (indicator.pointArrayMap || ['y']), true);
