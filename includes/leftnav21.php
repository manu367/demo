        ...
        <div class="col-sm-3 nav-side-menu" style="padding-left:0px;padding-right:0px;border: 1px solid #2e353d;">
            ...
            <div class="menu-list">
                ...
                if (isset($_SESSION['uname'])) { ?>
                    <span>Welcome <?php echo $_SESSION['uname']."  (".$_SESSION['userid'].")";?><br/>
                ...
                <ul id="menu-content" class="menu-content collapse out">
                    ...
                    if ($num_maintab > 0) { ///// start main tab if
                        ...
                        while($row_maintab=mysqli_fetch_array($res_maintab)){ ////// start main tab while
                            $icon = $row_maintab['maintabicon'];
                            ?>
                            <li data-toggle="collapse" data-target="#products<?=$i?>" class="collapsed <?php
                            if($_REQUEST['hid'] == $row_maintab['maintabname']){ echo "active";} ?>">
                                <a href="#">
                                    <i class="fa <?=$icon?> fa-lg"></i>
                                    <?=$row_maintab['maintabname']?> <span class="arrow"></span></a>
                            </li>
                            <ul <?php
                            if($_REQUEST['hid'] == $row_maintab['maintabname'])
                            { echo "class='collapsed'";}
                            else{ echo "class='collapse'";} ?> id="products<?=$i?>">
                                <?php
                                    ...
                                    while($row_subtab=mysqli_fetch_array($res_subtab)){
                                        if($tab_array[$row_subtab['tabid']] == 1){
                                            ...
                                            <a href="<?=$row_subtab['filename'];?>.php?pid=<?=$row_subtab['tabid']?>&hid=<?=$row_maintab['maintabname']?>" style="color:white; text-decoration: none; ">
                                                <li <?php
                                                if($_REQUEST['pid']==$row_subtab['tabid'] && $_REQUEST['hid'] == $row_maintab['maintabname']) { echo "class='active'";} ?>>&nbsp;
                                                    <i class="fa <?=$row_subtab['subtabicon']?> fa-lg"></i>&nbsp;
                                                    <?=$row_subtab['subtabname']?></li>
                                            </a>
                                            <?php
                                        }
                                    }///// end sub tab while
            ...
            <script>
                document.querySelectorAll(".collapsed").forEach((li)=>{
