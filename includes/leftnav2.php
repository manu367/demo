    ...
    <ul class="nav navbar-nav ">
      ...
      if($_REQUEST['hid'] == $row_maintab['maintabname']){ echo "active";} ?>">
        ...
        <?php 
              ...
              $tab_query="select tabid , subtabname , subtabicon , filename from tab_master where status = '1' and tabfor='admin' and  maintabname = '".$row_maintab['maintabname']."' order by subtabseq";
	           $res_subtab=mysqli_query($link1,$tab_query);
    ...
    <div class="col-sm-3 nav-side-menu" style="padding-left:0px;padding-right:0px;border: 1px solid #2e353d;">
        ...
        <div class="menu-list">
            <?php if (isset($_SESSION['uname'])) { ?>
                <span>
                Welcome <?php echo $_SESSION['uname']." (".$_SESSION['userid'].")";?><br/>
            ...
            <ul id="menu-content" class="menu-content">
                ...
                <li <?php
                if($_REQUEST['pid']=="home" && $_REQUEST['hid']=="home"){ echo "class='active'";}
                if($_SESSION['utype']=="ASP"){ $hmdir="asp";}else{ $hmdir="admin";}
                ?>>
                ...
                if (mysqli_num_rows($res_maintab) > 0) {
                    ...
                    while($row_maintab=mysqli_fetch_array($res_maintab)){
                        ...
                        <li data-toggle="collapse" data-target="#products<?=$i?>"
                            class="collapsed <?php if($_REQUEST['hid'] == $row_maintab['maintabname']) echo "active"; ?>">

                            <a href="#">
                                <i class="fa <?=$icon?> fa-lg"></i>
                                <?=$row_maintab['maintabname']?> <span class="arrow"></span>
                            </a>
                        </li>

                        <ul id="products<?=$i?>"
                            class="<?php echo ($_REQUEST['hid'] == $row_maintab['maintabname']) ? 'collapsed' : 'collapse'; ?>">
                            ...
                            if (mysqli_num_rows($res_subtab) > 0) {
